...
import { EventRespDTO, Invite, Event } from './dto/event.resp.dto';
...
export class AppService {
  ...
  async getEvent(): Promise<{ event: Event; invites: Invite[] }> {
    return {
      event,
      invites,
    };
  ...
  testTransFrom(name: String) {
    switch (name) {
      ...
      case this.TEST_TYPE.EXPOSE_DO_EXCLUDE:
        {
          let result = plainToClass(
            EventRespDTO,
            { event, invites },
            { groups: ['admin'] },
          );
          console.log(
            'Invite 排除了 email， 并且 admin 暴露了user的emails',
            // result.getInvite(),
          );
          ...
          console.log(
            'Invite 排除了email， user 因为组别不对 所以隐藏了email',
            // result.getInvite(),
          );
          console.log(
            '最终再把数据还原，进行返回',
            classToPlain(result).invites,
          );
        }
        break;
